...
<head>
    ...
    <title>@yield('title', 'Promotor Dashboard | Carnival')</title>
...
<body>
            ...
            <div style="display:flex; align-items:center; gap:16px;">
                <div style="display:flex; align-items:center; gap:12px;">
                    <div style="text-align:right;">
                        <div style="color:var(--text); font-weight:700; font-size:14px; font-family:var(--body);">{{ Auth::user()->promotor->company_name ?? explode(' ', Auth::user()->name)[0] }}</div>
                        <div style="color:var(--text-muted); font-size:11px; font-family:var(--mono); text-transform:uppercase;">Promotor</div>
                    </div>
                    <div style="width:36px; height:36px; border-radius:50%; background:linear-gradient(135deg, var(--accent), var(--accent-2)); color:#0A0A0F; display:flex; align-items:center; justify-content:center; font-weight:800; font-family:var(--display); font-size:18px;">
                        {{ strtoupper(substr(Auth::user()->promotor->company_name ?? Auth::user()->name, 0, 1)) }}
                    </div>
                </div>
    ...
    <div class="page active" id="promoter">
        <div class="dash-shell">
            ...
            @include('components.promotor-sidebar')
