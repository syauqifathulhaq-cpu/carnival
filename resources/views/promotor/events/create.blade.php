...
@section('title', 'Buat Event Baru | Carnival')
...
<div class="dash-topbar" style="margin-bottom: 24px;">
    <h2 style="font-family:var(--display); font-size:24px; text-transform:uppercase;">Buat Event Konser Baru</h2>
</div>

<div class="form-card" style="max-width: 800px;">
    <form action="{{ route('promotor.events.store') }}" method="POST" enctype="multipart/form-data">
        @csrf
        
        <div class="field" style="margin-bottom: 16px;">
            <label>Nama Konser / Event</label>
            <input type="text" name="event_name" required placeholder="Contoh: Coldplay Music of The Spheres">
        </div>
        ...
        <div class="form-row">
            <div class="field">
                <label>Kota</label>
                <input type="text" id="city_input" name="city" required placeholder="Contoh: Bandung">
            </div>
            <div class="field">
                <label>Lokasi / Venue</label>
                <input type="text" id="location_input" name="location" required placeholder="Contoh: Stadion Siliwangi">
            </div>
        </div>
        ...
        <div class="field" style="margin-bottom: 16px;">
            ...
            <textarea name="description" rows="4" style="background:var(--bg-elevated); border:1px solid var(--border); border-radius:8px; padding:12px 14px; color:var(--text); font-family:var(--body); font-size:14px; width:100%;" placeholder="Jelaskan mengenai konser ini..."></textarea>
        </div>

        <div class="form-row" style="margin-bottom: 32px;">
            <div class="field">
                <label>Max Tiket Per NIK</label>
                <input type="number" name="max_tickets_per_nik" value="4" min="1" required>
            </div>
            <div class="field">
                <label>Poster/Banner (Opsional)</label>
                <input type="file" name="banner_image" accept="image/*" style="margin-bottom: 1rem;">
                
                <label>Denah Kursi / Seatmap (Opsional)</label>
                <input type="file" name="seatmap_image" accept="image/*">
            </div>
        </div>
        
        <button type="submit" class="btn btn-primary btn-block">LANJUT BUAT KATEGORI TIKET</button>
    </form>
</div>
...
<script>
    ...
    function openMapModal() {
        ...
        if (!map) {
            // Default to Jakarta
            map = L.map('leafletMap').setView([-6.2088, 106.8456], 12);
        ...
        // Fix map rendering issue when initialized in a hidden div
        setTimeout(() => {
            map.invalidateSize();
        }, 100);
    }
